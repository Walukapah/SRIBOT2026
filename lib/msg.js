...
const downloadMediaMessage = async(m, filename) => {
	...
	if (m.type === 'imageMessage' || m.type === 'imageMessageWithButtons') { // Added imageMessageWithButtons
	...
	if (m.message) {
		...
		if (m.msg) {
			...
			m.body = (m.type === 'conversation') ? m.msg : 
					 ...
					 (m.type === 'listResponseMessage') && m.msg.singleSelectReply ? m.msg.singleSelectReply.selectedRowId :
					 (m.type === 'listResponseMessage') && m.msg.multiSelectReply ? m.msg.multiSelectReply.selectedRowId :
					 (m.type === 'imageMessageWithButtons') && m.msg.caption ? m.msg.caption : // Added imageMessageWithButtons
					 (m.type === 'imageMessageWithButtons') && m.msg.selectedId ? m.msg.selectedId : // Handle button response
					 '';
