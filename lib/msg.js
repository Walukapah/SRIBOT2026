...
const sms = (conn, m) => {
	...
	if (m.message) {
		...
		if (m.msg) {
			...
			m.body = (m.type === 'conversation') ? m.msg : (m.type === 'extendedTextMessage') ? m.msg.text : (m.type == 'imageMessage') && m.msg.caption ? m.msg.caption : (m.type == 'videoMessage') && m.msg.caption ? m.msg.caption : (m.type == 'templateButtonReplyMessage') && m.msg.selectedId ? m.msg.selectedId : (m.type == 'buttonsResponseMessage') && m.msg.selectedButtonId ? m.msg.selectedButtonId : (m.type == 'listResponseMessage') && m.msg.singleSelectReply ? m.msg.singleSelectReply.selectedRowId : (m.type == 'interactiveResponseMessage') && m.msg.nativeFlowResponseMessage ? JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson).id : ''
	...
	// Button message functions - UPDATED FOR BETTER BUTTON SUPPORT
	m.replyButtons = (text, buttons, title, footer, id = m.chat) => {
		...
		return conn.sendMessage(id, buttonMessages, { quoted: m })
	}
	...
	// New function for sending interactive messages with buttons
	m.replyInteractive = (text, buttons, title, footer, id = m.chat) => {
	...
	// Function to send media with buttons
	m.replyMediaWithButtons = async (media, type, text, buttons, title, footer, id = m.chat) => {
		let mediaContent;
